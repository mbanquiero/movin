function mostrarGrafico(id) {
    ...
    const data = {
        datasets: [
            {
                label: id, // EN FORMA TEXTO PODRÍA SER "Valor_CO2" (SUSTITUIDO POR VARIABLE "id" PASADA COMO PARÁMETRO) ¿???
                data: [],
                backgroundColor: "rgba(54, 162, 235, 0.2)",
                borderColor: "rgba(75, 192, 192, 1)",
                borderWidth: 1,
                tension: 0.5, // MEJORA ESTÉTICA PROPUESTA ¿???
                fill: true, // MEJORA ESTÉTICA PROPUESTA ¿???
                pointBorderWidth: 4, // MEJORA ESTÉTICA PROPUESTA ¿???
            }
        ]
    };
    const config = {
        ...
        options: {
            ...
            scales: {
                    ...
                    realtime: {
                        ...
                        onRefresh: chart => {
                            ...
                            chart.data.datasets[0].data.push({
                                x: now,
                                y: MIN_CO2, // ¿??? CON VALORES RANDOM DE PRUEBA HA FUNCIONADO ( ¿ AQUÍ IRÍA: geneticAlgorithm(populationSize, individualLength, mutationRate, generations); ??? )
                                iterac: simu
                            });
                ...
                y: {
                    beginAtZero: true,
                    min: 0,
                    max: 1000 // QUIZÁ HAYA QUE PONER UN VALOR MÁXIMO MÁS ALTO ( P.EJ. 1000 ¿??? )
                },
            },
        },
    };
